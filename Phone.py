...
Phone=pd.read_csv("C:\\Data Science\\Assignments\\Module-Association Rules\\Codes\\myphonedata.csv",sep=",")
Phone = Phone.iloc[:,[0,1,2,]]
### Considering only the transactions, i.e the textual transactions.

#### Imputation to convert the nan values to 0's##
Phone.iloc[:,1:2] = Phone.iloc[:,1:2].apply(lambda x:x.fillna(0))

X = pd.get_dummies(Phone[['V1','V2','V3']])
...
x_without_dum = X.iloc[:,[0,1,2,3,5,6,7,8,10,]]
